...
class InputStatus extends BaseComponent
{
    ...
    public function store()
    {
        if ($this->awb->isDelivered()) {
            $this->emit('error', 'Cannot change status of already delivered AWB');
            $this->emit('close-modal', '#modal-input-status-awb-'.$this->uuid);
        ...
        $this->emit('message', 'AWB '.$awb.' status successfully changed');
        $this->emit('close-modal', '#modal-input-status-awb-'.$this->uuid);
